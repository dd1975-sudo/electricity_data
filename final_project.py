...
# START: fix errors with data
# Print the first few rows to inspect the data (including header issue)
print(df_el.head())

# Try to convert 'Energy (kWh)' to numeric, coercing any errors (non-numeric to NaN)
df_el['Energy (kWh)'] = pd.to_numeric(df_el['Energy (kWh)'], errors='coerce')

# Check for any NaN values in the column (indicating non-numeric data)
non_numeric_values = df_el[df_el['Energy (kWh)'].isnull()]
if not non_numeric_values.empty:
    print("Found non-numeric values in 'Energy (kWh)':")
    print(non_numeric_values)

# Option 1: Drop rows with NaN values (i.e., where conversion failed)
df_el = df_el.dropna(subset=['Energy (kWh)'])

# Option 2 (Alternative): Fill NaN values with a default value (e.g., 0) instead of dropping
# df_el['Energy (kWh)'].fillna(0, inplace=True)

# Now, convert the cleaned column to float
df_el['Energy (kWh)'] = df_el['Energy (kWh)'].astype(float)

# Print the data types to confirm
print(df_el.dtypes)

# Print the cleaned data to verify successful conversion
print(df_el.head())
